...
namespace LoadDataCalc
{
    ...
    public class Fiducial_Single_Displacement : BaseInstrument
    {
        ...
        {
            DataTable dt = new DataTable();
            string TableName = "Table_Result_SM";
            dt.TableName = TableName;
            dt.Columns.Add("ID");
            ...
            foreach (PointSurveyData pd in datas)
            {
                foreach (var surveydata in pd.Datas)
                {
                    ...
                    dr["FREQ"] = Math.Round(surveydata.Survey_ZorR, 1);
                    dr["TEMPC"] = Math.Round(surveydata.Survey_RorT, 2);
                    dr["Temperature"] = Math.Round(surveydata.Tempreture, 2);
                    dr["Disp"] = Math.Round(surveydata.LoadReading, 3);
    ...
    public class Fiducial_Hose_Settlement_Gauge : BaseInstrument
    {
        ...
        public override double ShakeString(ParamData param,SurveyData data, params double[] expand)
        {
            double result = (param.ZorR-data.Survey_ZorR) * param.Gorf;
            data.LoadReading = result;
            data.Survey_ZorRMoshu = data.Survey_ZorR;
            return result;
        ...
        public override ParamData GetParam(string Survey_point_Number, string tablename = null)
        {
            string sql = @"select * from {0} where Instrument_Serial='{1}'";
            sql = string.Format(sql, tablename, Survey_point_Number);
            var SqlHelper = CSqlServerHelper.GetInstance();
            var dt = SqlHelper.SelectData(sql);
            if (dt.Rows.Count < 1) return null;
            try
            {
                ParamData pd = new ParamData();
                pd.SurveyPoint = Survey_point_Number;
                pd.Gorf = ConvetToData(dt.Rows[0]["Standard"]);
                pd.ZorR = ConvetToData(dt.Rows[0]["BData"]);
                return pd;
            }
            catch
            {
        ...
        public override int WriteSurveyToDB(List<PointSurveyData> datas)
        {
            ...
            dt.Columns.Add("Observation_Date");
            dt.Columns.Add("Observation_Time");
            dt.Columns.Add("Voltage1");
            dt.Columns.Add("Voltage2");
    ...
    public class Fiducial_Measure_Aperture : BaseInstrument
    {
        ...
        {
            DataTable dt = new DataTable();
            string TableName = "Table_Result_J";
            dt.TableName = TableName;
            dt.Columns.Add("ID");
            dt.Columns.Add("Point_Number");
            dt.Columns.Add("Observation_Date");
    ...
    public class Fiducial_Leakage_Pressure : BaseInstrument
    {
        ...
        public override ParamData GetParam(string Survey_point_Number, string tablename=null)
        {
            ...
            try
            {
                ...
                pd.ZorR = ConvetToData(dt.Rows[0]["Benchmark_Resist_Ratio"]);
                pd.Elevation = ConvetToData(dt.Rows[0]["Elevation"]);
                if (dt.Rows[0]["Tempera_Revise_K"] == null)
                {
                    pd.Korb = 0;
                }
        ...
        {
           DataTable dt = new DataTable();
           string TableName = "Table_Result_P";
            dt.TableName = TableName;
            dt.Columns.Add("ID");
            dt.Columns.Add("Point_Number");
            dt.Columns.Add("Observation_Date");
            ...
            foreach (PointSurveyData pd in datas)
            {
                foreach (var surveydata in pd.Datas)
                {
                    ...
                    dr["Press"] = Math.Round(surveydata.LoadReading,3);
                    dr["ST"] = Math.Round(surveydata.ResultReading,3);
                    dr["SW"] = Math.Round(surveydata.SW,3);
                    if (Encoding.Default.GetBytes(surveydata.Remark).Length > 60)
                    {
                        surveydata.Remark = "";
                    }
    ...
    public class Fiducial_MeasureWater_Weir : BaseInstrument
    {
        ...
        public override double ShakeString(ParamData param,SurveyData data, params double[] expand)
        {
            //1.86*B12*SQRT(POWER(C12,3))
            data.LoadReading = 1.86 * param.Gorf * Math.Pow(data.Survey_ZorR, 1.5);
            data.ResultReading = data.LoadReading*1000;
        ...
        {
            //F518*2*SQRT(2*9.81)*$B$3*I518^1.5*1000/3
            double result = (0.402 + 0.054 * (data.Survey_ZorR - param.ZorR) / param.Korb);
            return base.Expand1(param, data, expression);
        }
        public override ParamData GetParam(string Survey_point_Number, string tablename = null)
        {
            string sql = @"select * from {0} where Survey_point_Number='{1}'";
            ...
            try
            {
                ParamData pd = new ParamData();
                pd.SurveyPoint = Survey_point_Number;
                pd.Gorf = ConvetToData(dt.Rows[0]["A1_C"]);
                pd.Korb = ConvetToData(dt.Rows[0]["A2_C"]);
                pd.ZorR = ConvetToData(dt.Rows[0]["Weir_Size"]);
        ...
        public override int WriteSurveyToDB(List<PointSurveyData> datas)
        {
            ...
            dt.Columns.Add("Observation_Time");
            dt.Columns.Add("Temperature");
            dt.Columns.Add("Water_Temperature");
    ...
    public class Fiducial_Strain_Gauge : BaseInstrument
    {
        ...
        public override double DifBlock(ParamData param,SurveyData data, params double[] expand)
        {
            ...
            if (param.ZeroR > 1&&data.Survey_RorT>0)
            {
            ...
            data.LoadReading = result;
            if (param.IsHasNonStress&&data.NonStressSurveyData != null && Math.Abs(data.Survey_ZorR) > 1)
            {
        ...
        {
            DataTable dt = new DataTable();
            string TableName = "Table_Result_S";
            dt.TableName = TableName;
            dt.Columns.Add("ID");
            dt.Columns.Add("Point_Number");
            dt.Columns.Add("Observation_Date");
            ...
            foreach (PointSurveyData pd in datas)
            {
                foreach (var surveydata in pd.Datas)
                {
                    ...
                    dr["FREQ"] = Math.Round(surveydata.Survey_ZorR, 1);
                    dr["TEMPC"] = Math.Round(surveydata.Survey_RorT, 2);
                    dr["Temperature"] = Math.Round(surveydata.Tempreture, 3);
                    dr["ResultPress"] = Math.Round(surveydata.ResultReading, 3);
                    if (Encoding.Default.GetBytes(surveydata.Remark).Length > 60)
                    {
                        surveydata.Remark = "";
                    }
    ...
    public class Fiducial_Nonstress : BaseInstrument
    {
        ...
        public override double DifBlock(ParamData param,SurveyData data, params double[] expand)
        {
            ...
            data.ResultReading = result;
            data.Tempreture = data.Survey_RorT == 0 ? 0 : param.TemperatureRead * (data.Survey_RorT - param.ZeroR);
            data.LoadReading = result;
            data.Survey_ZorRMoshu = data.Survey_ZorR;
            return result;
        }
        ...
        {
            DataTable dt = new DataTable();
            string TableName = "Table_Result_N";
            dt.TableName = TableName;
            dt.Columns.Add("ID");
            dt.Columns.Add("Point_Number");
            dt.Columns.Add("Observation_Date");
    ...
    public class Fiducial_Steel_Bar : BaseInstrument
    {
        ...
        {
            DataTable dt = new DataTable();
            string TableName = "Table_Result_R";
            dt.TableName = TableName;
            dt.Columns.Add("ID");
            dt.Columns.Add("Point_Number");
            dt.Columns.Add("Observation_Date");
    ...
    public class Fiducial_Anchor_Pole : BaseInstrument
    {
        ...
        public override double DifBlock(ParamData param,SurveyData data, params double[] expand)
        {
            ...
            data.LoadReading = result;
            data.Survey_ZorRMoshu = data.Survey_ZorR;
            data.ResultReading = (param.Steel_Diameter_L != 0) ? result * 1000 / (Math.PI * Math.Pow(param.Steel_Diameter_L / 2.0, 2)) : data.LoadReading;
            return result;
        }
        public override double ShakeString(ParamData param,SurveyData data, params double[] expand)
        {
        ...
        {
            DataTable dt = new DataTable();
            string TableName = "Table_Result_AS";
            dt.TableName = TableName;
            dt.Columns.Add("ID");
            dt.Columns.Add("Point_Number");
            dt.Columns.Add("Observation_Date");
    ...
    public class Fiducial_Anchor_Cable : BaseInstrument
    {
        ...
        {
            DataTable dt = new DataTable();
            string TableName = "Table_Result_PR";
            dt.TableName = TableName;
            dt.Columns.Add("ID");
            dt.Columns.Add("Point_Number");
            dt.Columns.Add("Observation_Date");
            ...
            dt.Columns.Add("AVERAGE");
            dt.Columns.Add("TEMPC1");
            dt.Columns.Add("Temperature1");
            ...
            foreach (PointSurveyData pd in datas)
            {
                foreach (var surveydata in pd.Datas)
                {
                    ...
                    dr["TEMPC1"] = Math.Round(surveydata.Survey_RorT, 2);
                    dr["Temperature1"] = Math.Round(surveydata.Tempreture, 2);
                    dr["AVERAGE"] = Convert.ToDecimal(surveydata.clacAverage);
                    dr["Result1"] = Convert.ToDecimal(surveydata.LoadReading);
                    dr["AfterLock"] = Convert.ToDecimal(surveydata.AfterLock);
                    dr["PlanLock"] = Convert.ToDecimal(surveydata.PlanLock);
                    dr["ResultLock"] = Convert.ToDecimal(surveydata.ResultLock);
                    if (Encoding.Default.GetBytes(surveydata.Remark).Length > 60)
    ...
    public class Fiducial_Armor_plate : BaseInstrument
    {
        ...
        public override double ShakeString(ParamData param,SurveyData data, params double[] expand)
        {
            ...
            {
                result = param.Gorf * (data.Survey_ZorR * data.Survey_ZorR - param.ZorR * param.ZorR) + tcorrect;
            }
        ...
        {
            DataTable dt = new DataTable();
            string TableName = "Table_Result_GB";
            dt.TableName = TableName;
            dt.Columns.Add("ID");
            dt.Columns.Add("Point_Number");
            dt.Columns.Add("Observation_Date");
    ...
    public class Fiducial_Temperature : BaseInstrument
    {
        ...
        public override int WriteSurveyToDB(List<PointSurveyData> datas)
        {
            ...
            dt.Columns.Add("Observation_Time");
            dt.Columns.Add("Reading");
            dt.Columns.Add("Real_Temperature");
            dt.Columns.Add("Remark");
            dt.Columns.Add("UpdateTime");
            if (Config.IsAuto) dt.Columns.Add("RecordMethod");
            var sqlhelper = CSqlServerHelper.GetInstance();
        ...
        {
            DataTable dt = new DataTable();
            string TableName = "Table_Result_T";
            dt.TableName = TableName;
            dt.Columns.Add("ID");
            dt.Columns.Add("Point_Number");
            dt.Columns.Add("Observation_Date");
            ...
            foreach (PointSurveyData pd in datas)
            {
                foreach (var surveydata in pd.Datas)
                {
                    ...
                    dr["Result"] = Math.Round(surveydata.LoadReading, 1);
                    if (Encoding.Default.GetBytes(surveydata.Remark).Length > 60)
                    {
                        surveydata.Remark = "";
                    }
    ...
    public class Fiducial_Water_Level_Hole : BaseInstrument
    {
        public Fiducial_Water_Level_Hole()
        {
        ...
        public override double ShakeString(ParamData param,SurveyData data, params double[] expand)
        {
            if (data.Survey_RorT != 0)
            {
                data.LoadReading = data.Survey_RorT - data.Survey_ZorR;
            }
            else
            {
                data.Survey_RorT = param.RorT;
                data.LoadReading = param.RorT - data.Survey_ZorR;
            }
       
            data.ResultReading = data.LoadReading - expand[0];
        ...
        {
            var sqlhelper = CSqlServerHelper.GetInstance();
            string sql = "SELECT Srart_PipeElev from Fiducial_Water_Level_Hole where Survey_point_Number='{0}' ";
            var result = sqlhelper.SelectFirst(string.Format(sql, Survey_point_Number));
            var param = new ParamData();
            if (result != DBNull.Value)
            {
                param.SurveyPoint = Survey_point_Number;
                param.RorT = Convert.ToDouble(result);
        ...
        public override int WriteResultToDB(List<PointSurveyData> datas)
        {
            ...
            dt.Columns.Add("Observation_Date");
            dt.Columns.Add("Observation_Time");
            dt.Columns.Add("Water_Level");
            dt.Columns.Add("Water_Level_Change");
            dt.Columns.Add("Remark");
            dt.Columns.Add("UpdateTime");
            if (Config.IsAuto) dt.Columns.Add("RecordMethod");
            var sqlhelper = CSqlServerHelper.GetInstance();
